...
const messageInput = document.getElementById('message-input');
    ...
    function createMessageElement(content, type = 'chipper', timestamp) {
      ...
      switch (type) {
        case 'user':
          messageDiv.classList.add('bg-amber-500', 'text-white', 'user-message');
          break;
        case 'chipper':
          messageDiv.classList.add('bg-gray-200', 'chipper-message');
          break;
        case 'error':
          messageDiv.classList.add('bg-red-500', 'text-white', 'error-message');
          break;
        case 'system':
          messageDiv.classList.add('bg-purple-500', 'text-white', 'system-message');
          break;
        default:
    ...
    async function handleCommand(command) {
      ...
      switch (parts[0]) {
        ...
        case '/context': {
          ...
          if (isNaN(size)) {
            chatMessages.appendChild(
              createMessageElement(
                'Please provide a valid number for context size',
                'error'
              )
            );
            scrollToBottom();
          ...
          try {
            const response = await fetch('/api/context-size', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
              },
            ...
            chatMessages.appendChild(
              createMessageElement(`Context size updated to ${size}`, 'system')
            );
            scrollToBottom();
          } catch (error) {
            chatMessages.appendChild(
        ...
        default:
          chatMessages.appendChild(
            createMessageElement(`Unknown command: ${command}`, 'error')
          );
          scrollToBottom();
      }
    }